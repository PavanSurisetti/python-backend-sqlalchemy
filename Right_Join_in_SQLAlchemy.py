...
engine=create_engine('sqlite:///Rightjoin.db')
...
print('======'*3)
#now let's perform Right join
result=session.query(course,Student).outerjoin(Student).all()
#.outerjoin(Course) → RIGHT JOIN u have to interchange the tables 
for c,s in result:
    print(s.id if s else None,s.name if s else None, c.id , c.cname )
