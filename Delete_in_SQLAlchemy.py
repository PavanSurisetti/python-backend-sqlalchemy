...
session.add_all([s2,s3,s5])
session.commit()
print('DataBase Contains Before Delete:')
...
if temp:
    session.delete(temp)
    session.commit()
    print('DataBase Contains After Delete:')
    print('Sid \t Sname \t Sage \t sper')
    data=session.query(Student).all()
    for i in data:
        print(i.sid,'\t',i.sname,'\t',i.sage,'\t',i.sper)
...
#multiple delete
session.query(Student).filter(Student.sage==20).delete()
session.commit()
print('DataBase Contains After Multiple Deletes:')
print('Sid \t Sname \t Sage \t sper')
data=session.query(Student).all()
for i in data:
    print(i.sid,'\t',i.sname,'\t',i.sage,'\t',i.sper)