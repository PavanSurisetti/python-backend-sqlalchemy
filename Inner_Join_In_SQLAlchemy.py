...
engine=create_engine('sqlite:///Innerjoin.db')
...
print('======'*3)
#now let's perform inner join
result=session.query(Student,course).join(course).all()
#.join(Course) → INNER JOIN
for s,c in result:
    print(s.id,s.name,c.id,c.cname)
